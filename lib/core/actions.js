...
const createProjectAction = async (project) => {
    console.log('why helps you create your project...'.blue);

    //1.clone项目 download(clone地址，存放的目录，是否clone所有文件内容)
    await download(vueRepo, project);

    //2.执行npm install(windows电脑上要写'npm.cmd',Mac上则不用)
    console.log('开始执行npm install...'.green);
    const command = process.platform === 'win32' ? 'npm.cmd' : 'npm';
    await commandSpawn(command, ['install'], { cwd: `./${project}` });

    //3.运行npm run serve
    console.log('开始运行npm run serve...'.red);
    commandSpawn(command, ['run', 'serve:dev'], { cwd: `./${project}` }).then();
    //注意：npm run serve命令执行时该子进程不会结束，只有按下ctrl+c才会结束进程，所以主进程在这里被阻塞了。所以不使用await
